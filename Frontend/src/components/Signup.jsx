...
function Signup() {
    ...
    // watch the password and confirm passwordfields
    ...
    const onSubmit = async (data) => {
        ...
        await axios.post("/api/user/signup", userInfo)
            .then((response) => {
                ...
                localStorage.setItem("ChatNow", JSON.stringify(response.data))
    ...
    return (
        <>
            <div className="flex h-screen bg-slate-900 text-gray-300 items-center justify-center">
                <form onSubmit={handleSubmit(onSubmit)} className="border border-white px-6 py-2 space-y-3 rounded-md w-96">
                    ...
                    <label className="input input-bordered border-gray-700 bg-slate-900 text-gray-300 flex items-center gap-2">
                        <svg
                    ...
                    {errors.confirmPassword && <span className='text-sm text-red-500 font-semibold'>{errors.confirmPassword.message}</span>}
                    {/* text and button */}
